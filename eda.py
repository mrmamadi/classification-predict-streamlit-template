...
def wordFrequencyDict(df = train_data, target='target', vocab = getVocab()):
    """
...
def getClassWords(word_frequency_dict = wordFrequencyDict()):
    """
    doctstring
    Parameters
    ----------
...
def getOrder(class_words = getClassWords()):
    """
...
def topNWords(ordered_words = getOrder(), n = 5000):
    """
    count total vocabulary from Dataframe message
...
def removeInfrequentWords(tweet, include = topNWords()):
    """
...
def allVocab(df=train_data, target = 'tweets_clean'):
    all_vocab = list()
...
def plotDist(df=train_data, target = 'len_of_tweet'):
    sns.distplot(train_data['len_of_tweet'])
    plt.show()

def plotCounts(df=train_data):
...
def applyScores(data = train_data):
...
def getPolaritySubjectivity(data = train_data):
...
def violinPlots(data=train_data, target = 'target'):
...
def plotAltScatter(df = train_data, X = 'compound', y_ = 'polarity', title_ = 'Compound Vs Polarity\n'):
...
def arrowScatter(df = train_data):
    fig = plt.figure(figsize = (8, 5))
    g = sns.scatterplot(data = df, x = 'subjectivity', y = 'polarity', color = 'teal', hue = 'target', alpha = 1/3)
...
def polCompNeutralPlot(df = train_data):
...
def polPlusCompScatter(df = train_data):
...
def midCloud(df = train_data, lower = -0.15, inteval = 0.3, sentiment = [-1,0,1,2]):
...
def upperCloud(df = train_data, upper = 0.6, inteval = 0.3, sentiment = [-1,0,1,2]):
